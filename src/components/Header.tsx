...
function useBrandSettings() {
  ...
  useEffect(() => {
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data?.value?.logo_url) setNavLogo(data.value.logo_url);
      });
  }, []);
  return { navLogo };
}
...
export function useHeaderMedia() {
  ...
  useEffect(() => {
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (data?.value) setMedia(data.value);
      });
  }, []);

  useEffect(() => {
    if (!media || media.type !== "slideshow" || media.images.length <= 1)
      return;
    const timer = setInterval(
      () => {
        setCurrentIndex((i) => (i + 1) % media.images.length);
      },
      (media.interval || 3) * 1000,
    );
...
export function Header() {
  ...
  const { navLogo } = useBrandSettings();
  ...
  const LogoCenter = () => (
    ...
    >
      {navLogo ? (
        ...
        <img
          src={navLogo}
          alt="Royal Brands Fashion"
          className={cn(
            "object-contain transition-all duration-500 drop-shadow-md",
            isScrolled || !isHomePage ? "h-8 sm:h-9" : "h-10 sm:h-12",
          )}
        />
      ) : (
        // Default text logo
      ...
      <header
        className={cn(
          ...
          isScrolled || !isHomePage
            ...
            : "bg-gradient-to-b from-background/40 to-transparent py-2 md:py-4",
        )}
      >
          ...
          <div className="flex items-center gap-1 sm:gap-4 flex-1">
            ...
            <div className="flex items-center gap-1.5 sm:gap-4">
              ...
              <a
                ...
                className="text-cream hover:text-[#25D366] hover:scale-110 transition-all p-2 drop-shadow-md"
                aria-label="WhatsApp"
              >
                <svg
          ...
          <div className="flex items-center gap-3">
            {navLogo ? (
              ...
              <>
                ...
                <span className="font-display text-base tracking-widest text-cream uppercase font-semibold">
                  Royal Brands
                </span>
              </>
            )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 flex flex-col items-center">
          <div className="flex flex-col items-center gap-6 w-full max-w-sm">
            ...
            <NavLink
              to="/shop"
              onClick={() => setOpenMobile(false)}
              className="text-4xl sm:text-3xl font-display tracking-[0.05em] text-cream drop-shadow-md hover:text-primary transition-colors font-bold py-3 w-full text-center block rtl:tracking-normal"
            >
              {t.nav.shop}
            </NavLink>
            ...
            <NavLink
              to="/about"
              onClick={() => setOpenMobile(false)}
              className="text-3xl sm:text-2xl font-display tracking-[0.05em] text-cream drop-shadow-md hover:text-primary transition-colors font-bold py-3 w-full text-center block rtl:tracking-normal"
            >
              {t.nav.about}
            </NavLink>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 mt-6 text-cream hover:text-[#25D366] transition-all group"
              >
                <div className="p-3 rounded-full bg-primary/10 group-hover:bg-[#25D366]/20 transition-all">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                ...
                <span className="text-xs font-bold uppercase tracking-[0.2em] text-cream group-hover:text-primary transition-colors rtl:tracking-normal">
                  {locale === "ar" ? "تواصل معنا" : locale === "tr" ? "Bize Ulaşın" : "Contact Us"}
                </span>
              </a>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 w-full mt-10 pt-10 border-t border-border/5 text-center">
              ...
              >
                ...
                <span className="text-xs sm:text-sm uppercase tracking-widest font-bold text-cream drop-shadow-sm rtl:tracking-normal">
                  {t.wishlist.title}
                </span>
              </NavLink>
              <button
